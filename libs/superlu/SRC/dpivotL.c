...
if ( jcol == MIN_COL ) {
    ...
    if ( pivmax == 0.0 ) {
	/* This code results in a segfault, it's an error condition
		anyway so I just commeneted it out, I'm don't understand the
		SuperLU code enough to figure out what's going on. CE
	*pivrow = lsub_ptr[pivptr];
	perm_r[*pivrow] = jcol;
	*/
	printf("SuperLU Error: Signularity\n");
	*usepr = 0;
	return (jcol+1);
    }
